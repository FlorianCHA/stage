...
<p style="text-align:justify";>Après avoir masqué les séquences répétées dans tous les assemblages, une annotation est réalisée pour chaque assemblage L'annotation est réalisée avec l'outil BRAKER <a href='biblio.html#annotation1'>[5]</a>. Cet outil prend en input un fichier au format fasta contenant le génome à annoter et un fichier hints.Gff.</p>
<p style="text-align:justify";>Le fichier hints.gff va être obtenue par alignement de donnée RNAseq avec le genome mais aussi par alignement d'un fichier d'annotation protéique du génome provenant de 70-15. Pour cela une première étape est de réaliser un alignement des fichiers RNAseq sélectionnés avec chaque génome assemblé puis de crée le fichier hints à l'aide de l'outils bam2hints. La deuxième étape sera l'utilisation de l'outil exonerate pour aligné le fichier protéine sur le genome et récupéré les introns hints. La dernière étape sera de merge les deux fichier hints pour en former qu'un seul.</p>
...
<p style="text-align:justify";>Pour l'alignement l'outil TOGGLE <a href='biblio.html#annotation2'>[6]</a> est utilisé. Cette outils permet,ici, de lancer l'outils tophat sur tous les fichiers RNAseq disponible.L'outils donne en sortie un fichier unmapped.bam contenant les séquences qui ne se sont pas alignés avec le génome du riz. Ces fichiers bam sont récupérés puis convertis en fichier fastq grâce à l'outil samtool bam2fq.</p>
...
<p style="text-align:justify";>Pour l'alignement des 32 RNAseq sur chaque assemblage, l'outils TOGGLE va être utilisé afin de mapper chaque donnée RNAseq avec l'assemblage. Puis les données sont triées avec l'outil picard pour ensuite être tous mergé ensemble avec l'outil Samtools merge. Enfin pour pouvoir être utilisé par BRAKER, le fichier contenant tous les alignements entre les données RNAseq et le génome va être transformé à l'aide de l'outil Bam2hints et d'un script R.</p>
